        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12 index-contentBack">
                ...
                <div class="s-berita-lebih-banyak-container">
                    <?php
                    foreach ($citizenReport as $result) {
                        $result_title = seoUrl($result['News']['title_ind']);
                        if($result['News']['news_type_id'] == 1 || $result['News']['news_type_id'] == 3) {
                    ?>
                    <a href="<?= Router::url("/citizen-report/{$result['News']['id']}/$result_title", true) ?>" class="news-module-hor">
                    <?php
                        } else if($result['News']['news_type_id'] == 2) {
                    ?>
                    <a href="<?= Router::url("/citizen-report-foto/{$result['News']['id']}/$result_title", true) ?>" class="news-module-hor">
                    <?php
                        }
                    ?>
