...
class qqFileUploader {
    ...
    function __construct(array $allowedExtensions = array(), $sizeLimit = 10485760, $cakeForm = null) {
        ...
        if (isset($_GET['qqfile'])) {
            ...
            $this->file = new wonoUploadedForm($cakeForm);
        } else {
            $this->file = false;
        }
    }
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = FALSE) {
        if (!is_writable($uploadDirectory) && !mkdir($uploadDirectory, 0777, true)) {
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)) {
            return array('status' => 206, 'message' => 'Ok', "data" => array("fileName" => $filename . '.' . $ext, "folder" => $uploadDirectory));
        } else {
